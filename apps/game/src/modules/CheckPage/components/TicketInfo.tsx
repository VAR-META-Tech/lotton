...
const TicketInfo: FC<Props> = ({ name, activeRound, nameClassName, valueClassName, className, ...props }) => {
  return (
    <HStack pos={'apart'} {...props} className={cn('border border-primary', className)}>
      ...
      <p className={cn('text-right px-2 text-lg', valueClassName)}>Round {activeRound}</p>
    </HStack>
  );
};
